...
O=function(a){for(var c="",e=f.ticketList.items.length,d=0;d<e;d++)c=d==e-1?c+p(f.ticketList.items[d].section)+"**"+f.ticketList.items[d].price.toString():c+p(f.ticketList.items[d].section)+"**"+f.ticketList.items[d].price.toString()+",";c={l:C,s:c};var h=b(a);if(b.browser.msie&&window.XDomainRequest){var m=new XDomainRequest;m.open("post",A+"/getsectionlist-2");m.onprogress=function(){};m.onload=function(){var n=eval("["+m.responseText+"]");h.trigger("onReceiveSectionData",n)};m.send("l="+c.l+"&s="+
...
undefined){if(b.inArray(a,o)>-1){o=b.grep(o,function(B){return B!=a});b("#"+c.attr("id")).trigger("fvmapSectionDeselect",[g[a].pic+"?size="+f.ssize,g[a].name,a])}else if(!(parseFloat(g[a].minPrice)>parseFloat(v)||parseFloat(g[a].maxPrice)<parseFloat(u))){o=[a].concat(o);if(f.interactWithTicketList){var r=g[a].sections;b("#"+c.attr("id")).trigger("fvmapSectionSelect",[g[a].pic+"?size="+f.ssize,g[a].name,a,r])}else b("#"+c.attr("id")).trigger("fvmapSectionSelect",[g[a].pic+"?size="+f.ssize,g[a].name,
...
function(a){a=p(a);if(k[a]!=undefined)return g[k[a]].pic+"?size="+f.ssize;else{a=a.replace(" ","-").toLowerCase();if(l[a][0]!=undefined)return g[a].pic+"?size="+f.ssize}};b.fn.fanvenues.getOriginalSectionNamesFor=function(a){var c=[];for(s in k)k[s]==a&&c.push(s);return c};b.fn.fanvenues.getAllSections=function(){var a=[];for(i in g)a.push(i);return a};b.fn.fanvenues.defaults={version:"2.1.5",defaultZoom:2,height:500,width:500,mapSet:"fv",mapId:"1004",ticketList:{},rowSelector:"#tixlist div.tixlist-row",
sectionSelector:".psection",rowsToHideOnFilterSelector:"#tbl .pHeader",zoomMapButton:true,fullscreenMapButton:true,centerMapButton:true,resetMapButton:true,closeAllBubblesButton:true,printMapButton:true,priceFilter:true,interactWithTicketList:true,custom3d:"fanvenues",scrollWheelZoom:false,lsize:"745x400",ssize:"175x94",onReceiveVenueData:function(a,c,e,d){C=e;d="http://fanvenues3.appspot.com";H=d+"/tile/"+C+"/";g=c;N(this);O(this);f.interactWithTicketList&&P(this);a=document.createElement("ul");
new U(a,this);j.controls[google.maps.ControlPosition.TOP_RIGHT].push(a);if(f.priceFilter){a=document.createElement("div");new S(a);j.controls[google.maps.ControlPosition.BOTTOM].push(a)}if(f.zoomMapButton){a=document.createElement("ul");new T(a);j.controls[google.maps.ControlPosition.LEFT].push(a)}a=document.createElement("div");new R(a);j.controls[google.maps.ControlPosition.BOTTOM_RIGHT].push(a)},onReceiveSectionData:function(a,c){if(c!="NA"){for(s in c){var e=s.split("**"),d=p(s.split("**")[0]),
h=0;if(e.length>1)h=parseFloat(s.split("**")[1]);k[d]=c[s];if(g[k[d]]!=undefined){g[k[d]].section=d;for(e=0;e<f.ticketList.items.length;e++){var m=f.ticketList.items[e];if(m.section.toLowerCase()==d.toLowerCase())if(g[k[d]].sections==undefined)g[k[d]].sections=[m];else g[k[d]].sections.push(m)}if(g[k[d]].minPrice==undefined){g[k[d]].minPrice=h;g[k[d]].maxPrice=h}else{if(h<g[k[d]].minPrice)g[k[d]].minPrice=h;if(h>g[k[d]].maxPrice)g[k[d]].maxPrice=h}}}d=b(this);if(d.find(".section.focus").css("color")!=
